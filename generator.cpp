...
void intervalGenerator::printPubs()
{
	FILE* fp = NULL;
	fp = fopen("eventdata-rd.txt", "w");
	for(auto p : pubList){
		sort(p.pairs.begin(), p.pairs.end(), [](Pair a, Pair b){return a.att < b.att;});
		for(int i = 0;i < p.size-1; ++i)fprintf(fp, "%d ", p.pairs[i].value);
		fprintf(fp, "%d\n", p.pairs[p.size - 1].value);
	}
	fclose(fp);
}
...
IntervalSub intervalGenerator::GenOneSub(int id, int size, int atts, int attDis, int valDis, int valDom, double alpha,
	...
	if (attDis == 0)
		GenUniformAtts(sub, atts);
	else if (attDis == 1 || attDis == 2)
		GenZipfAtts(sub, atts, alpha);


	if (valDis == 0) // Uniform + fixed width
		GenUniformValues(sub);
	else if (valDis == 1) // Random width + Uniform >= width   ����һ�����˶����Ӿ��ȷֲ���������ȼ���width=0
