...
extern int valDis;			 // The distribution of values in subs and pubs. 0:uniform + fixed width | 1: Uniform + random width >= $width | 2: Zipf + fixed width | 3: Normal Distribution + random width both value ends
...
// Rein
extern int buks;				// Number of buckets.
...
extern int adarein_level;
// extern const double divider;
extern const double waitTimeUpperThreshold;
extern const double waitTimeLowerThreshold;
extern const int windowSize;
extern const double falsePositiveRateMax;
extern const double falsePositiveRateMaxVariation;
enum
{
	AdaRein_ORI,
